...
RSS_FEEDS = [
    ...
    ("Haaretz",           "https://www.haaretz.com/srv/haaretz-online/rss/index.xml"),
    ("i24 News",          "https://www.i24news.tv/en/rss"),
    ("Middle East Eye",   "https://www.middleeasteye.net/rss"),
]

LOOKBACK_HOURS = 36  # how far back to check for new articles
...
def fetch_feed(name, url, cutoff):
    ...
    try:
        req = Request(url, headers={
            'User-Agent': 'Mozilla/5.0 (compatible; StrikeTrackerBot/1.0)'
        })
...
def format_event_js(ev):
    ...
    return (
        ...
        f"   n:'{esc(ev['n'])}',\n"
        f"   sr:'{esc(ev['sr'])}',oc:'{esc(ev['oc'])}',tcc:'{esc(ev['tcc'])}'}},"
    )

def inject_events(html_content, new_events):
    """Append new events just before the closing ]; of the E array."""
    # Match: var E=[ ... ]; — the events array block
    m = re.search(r'(var E=\[[\s\S]*?)\];', html_content)
    if not m:
        print("  ✗ Could not locate events array — injection skipped")
        return html_content
