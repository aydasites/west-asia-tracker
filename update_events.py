...
RSS_FEEDS = [
    ...
    ("NNA Lebanon",       "https://nna-leb.gov.lb/en/rss"),
...
REGION_RX = re.compile(
    ...
    r'Tel Aviv|Haifa|Jerusalem|Eilat|Beirut|Dahieh|Dahiyeh|Tyre|Khiam|Litani|'
    r'Sanaa|Tehran|Isfahan|Natanz|Fordow|Baghdad|Erbil|Riyadh|Dubai|Jeddah|'
    r'Abu Dhabi|Manama|Doha|Kuwait City|Amman|Damascus|Aleppo|Hormuz|'
    r'Hodeidah|Bab al-Mandab|Camp Arifjan|Al Udeid)\b',
    re.IGNORECASE
)
...
GEO = {
    ...
    # Additional granular locations
    'Khiam':         [33.23, 35.56],  'Bint Jbeil':    [33.12, 35.43],
    'Kfar Kila':     [33.26, 35.56],  'Bekaa':         [33.85, 36.05],
    'Fordow':        [34.88, 51.58],  'Kharg Island':  [29.24, 50.33],
    'Minab':         [27.10, 57.08],  'Beit Shemesh':  [31.72, 34.99],
    'Camp Arifjan':  [29.07, 48.04],  'Al Udeid':      [25.12, 51.32],
    'Ras Tanura':    [26.64, 50.15],  'Karmiel':       [32.91, 35.30],
}
...
TARGET_COUNTRY_MAP = {
    ...
    'Beit Shemesh': 'Israel', 'Karmiel': 'Israel',
    'Iran': 'Iran', 'Tehran': 'Iran', 'Isfahan': 'Iran', 'Natanz': 'Iran',
    'Fordow': 'Iran', 'Minab': 'Iran', 'Kharg Island': 'Iran', 'Tabriz': 'Iran',
    'Yemen': 'Yemen', 'Sanaa': 'Yemen', 'Hodeidah': 'Yemen',
    'Lebanon': 'Lebanon', 'Beirut': 'Lebanon', 'Dahiyeh': 'Lebanon',
    'Tyre': 'Lebanon', 'Sidon': 'Lebanon', 'Baalbek': 'Lebanon', 'Bekaa': 'Lebanon',
    'Khiam': 'Lebanon', 'Bint Jbeil': 'Lebanon', 'Kfar Kila': 'Lebanon',
    'Syria': 'Syria', 'Damascus': 'Syria', 'Aleppo': 'Syria',
    'Iraq': 'Iraq', 'Baghdad': 'Iraq', 'Erbil': 'Iraq',
    'Jordan': 'Jordan', 'Amman': 'Jordan',
    'Saudi Arabia': 'Saudi Arabia', 'Riyadh': 'Saudi Arabia', 'Dhahran': 'Saudi Arabia', 'Aramco': 'Saudi Arabia', 'Ras Tanura': 'Saudi Arabia',
    'UAE': 'UAE', 'Dubai': 'UAE', 'Abu Dhabi': 'UAE', 'Sharjah': 'UAE',
    'Kuwait': 'Kuwait', 'Kuwait City': 'Kuwait', 'Camp Arifjan': 'Kuwait',
    'Qatar': 'Qatar', 'Doha': 'Qatar', 'Al Udeid': 'Qatar',
    'Bahrain': 'Bahrain', 'Manama': 'Bahrain',
    'Oman': 'Oman', 'Muscat': 'Oman',
    'Red Sea': 'Red Sea', 'Persian Gulf': 'Red Sea', 'Strait of Hormuz': 'Red Sea',
}
...
def classify_target(text):
    """Classify target type from article text, with shipping detection."""
    t = text.lower()
    if re.search(r'ship|vessel|tanker|cargo|bulk carrier|merchant|container ship|red sea.*attack|bab.?al.?mandab|maritime|commercial.*sea', t):
        return 'Commercial shipping'
    if re.search(r'oil refiner|oil facilit|petrochemical|pipeline|gas plant|energy infra|aramco|ras tanura|kharg', t):
        return 'Energy infrastructure'
    if re.search(r'airport|airfield|air base|airbase', t):
        return 'Airport / Military'
    if re.search(r'nuclear|natanz|fordow|enrichment|centrifuge', t):
        return 'Nuclear facilities'
    if re.search(r'naval|navy|warship|destroyer|frigate|port.*military', t):
        return 'Naval — military'
    if re.search(r'embassy|consulate|diplomatic', t):
        return 'Diplomatic'
    if re.search(r'school|hospital|mosque|church|residential|civilian', t):
        return 'Civilian infrastructure'
    if re.search(r'radar|air defense|air defence|s.?300|iron dome|patriot|thaad|missile defense', t):
        return 'Air Defense'
    if re.search(r'irgc|military|base|headquarter|command|weapon|ammo|arsenal|barracks|militia', t):
        return 'Military'
    if re.search(r'hezbollah|hamas|houthi', t):
        return 'Militia / armed group'
    return 'Unknown'
...
def fetch_feed(name, url, cutoff):
    ...
    try:
        req = Request(url, headers={
            'User-Agent': 'WestAsiaStrikeTracker/1.0 (https://aydasites.github.io/west-asia-striketracker; RSS feed reader)',
